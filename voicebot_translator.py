...
##### 기능 구현 함수 #####
def STT(speech, language):          # 추가 내용 : 언어 설정
    # 파일 저장
    filename='input.mp3'
...
def main():
    # 기본 설정
    st.set_page_config(
        page_title="음성 번역 챗봇 프로그램",
        layout="wide")

    # 제목 
    st.header("음성 번역 챗봇 프로그램")

    # 구분선
    st.markdown("---")

    # 기본 설명
    with st.expander("음성 번역 챗봇 프로그램에 관하여", expanded=True):
        st.write(
        """     
    ...
    system_content = "You are a thoughtful assistant. Respond to all input in 25 words or less."
